...
bundle's `CFBundleName` to `panea` and swaps in an icon generated by
`scripts/make-icon.mjs`. Only this project's `node_modules` is touched; re-run it
any time with `npm run brand`.
...
If the Dock still shows the old icon, macOS is serving a cached one:
`killall Dock` refreshes it.
